...
/* ------------------------------------------------------------------ */
/* metrics_tx_from_dumb_frame()                                        */
/* What:  Classify TX type by reading DUMB MsgType from the frame.     */
/* Why:   Retransmissions may use different fanout contexts; frame     */
/*        inspection is robust and counts every send.                  */
/* Notes: DUMB MsgType is stored at byte offset 14 in dumb_pack().     */
/* ------------------------------------------------------------------ */
#if DUMB_METRIC_LOG_ENABLED
static inline void
metrics_tx_from_dumb_frame(const uint8_t *buf, uint16_t len)
{
    if(buf == NULL || len < 15) return;
    if(buf[0] != 'D' || buf[1] != 'U' || buf[2] != 'M' || buf[3] != 'B') return;

    switch(buf[14]) {
    case DUMB_MSG_DATA:      metrics_agg_event(METR_EV_TX_DATA, 0, 0, 0, 0); break;
    case DUMB_MSG_ACK:       metrics_agg_event(METR_EV_TX_ACK,  0, 0, 0, 0); break;
    case DUMB_MSG_QUERY:     metrics_agg_event(METR_EV_TX_QUERY,0, 0, 0, 0); break;
    case DUMB_MSG_EMERGENCY: metrics_agg_event(METR_EV_TX_EMG,  0, 0, 0, 0); break;
    default: break;
    }
}
#endif // DUMB_METRIC_LOG_ENABLED
...
#if DUMB_METRIC_LOG_ENABLED
    metrics_tx_from_dumb_frame(ctx->buf, ctx->len);
#endif // DUMB_METRIC_LOG_ENABLED
...
/* Keep LED on if any emergency is active OR if force-window is active. */static void
ef_led_refresh(void)
{
...
#if DUMB_METRIC_LOG_ENABLED
        if(s->msg_type == DUMB_MSG_EMERGENCY)
...
send_emergency(uint8_t ef_code)
{
    ...
    uint8_t buf[DUMB_MAX_FRAME];
    uint16_t out_len = 0;
    if(!dumb_pack(&f, buf, sizeof(buf), &out_len)) return(0);
...
#else  // NOT DUMB_METRIC_LOG_ENABLED
            send_emergency(1);
#endif  // DUMB_METRIC_LOG_ENABLED
...
/* ------------------------------------------------------------------ */
static void
send_data_unicast_to_rsu(uint32_t xtrnl_msg_id)
{
    ...
    if(h.marker[0]=='R' && h.marker[1]=='S' && h.marker[2]=='U') {
        ...
        if(pl && pl_len >= 14) {
            ...
            if(ef_origin != 0) {
                ...
                else if(ef_code != DUMB_EF_NONE)   ef_add_or_update(ef_origin, ef_code, now_ms);
                ef_led_refresh();
            }
        }
...
#if DUMB_METRIC_LOG_ENABLED
        /* Metrics: per-message CREATE (batched) */
        metrics_agg_event(METR_EV_MBL_CREATE, origin_id, msg_id,
                          metrics_agg_now_ms(), 0);
#endif // DUMB_METRIC_LOG_ENABLED
