 ...
 *  - Emergency logic updated: harsh-braking update + TTL expiry (no FINISH)
 */
...
/* ------------------------------------------------------------------ */
#define BGKP_EF_BUTTON 2
#define EF_BUTTON_TIMEOUT_MS 3000
#define EF_BUTTON_TX_PERIOD_MS 1000

static uint8_t btn_pressed = 0;
//static uint32_t btn_last_tx_ms = 0;
...
/* ------------------------------------------------------------------ */
/* metrics_tx_from_bgkp_frame()                                        */
/* What:  Classify TX type by reading BGKP MsgType from the frame.     */
/* Why:   Retransmissions may use different fanout contexts; frame     */
/*        inspection is robust and counts every send.                  */
/* Notes: BGKP MsgType is stored at byte offset 14 in bgkp_pack().     */
/* ------------------------------------------------------------------ */
#if BGKP_METRIC_LOG_ENABLED
static inline void
metrics_tx_from_bgkp_frame(const uint8_t *buf, uint16_t len)
{
    if(buf == NULL || len < 15) return;
    if(buf[0] != 'B' || buf[1] != 'G' || buf[2] != 'K' || buf[3] != 'P') return;

    switch(buf[14]) {
    case BGKP_MSG_DATA:      metrics_agg_event(METR_EV_TX_DATA, 0, 0, 0, 0); break;
    case BGKP_MSG_ACK:       metrics_agg_event(METR_EV_TX_ACK,  0, 0, 0, 0); break;
    case BGKP_MSG_QUERY:     metrics_agg_event(METR_EV_TX_QUERY,0, 0, 0, 0); break;
    case BGKP_MSG_EMERGENCY: metrics_agg_event(METR_EV_TX_EMG,  0, 0, 0, 0); break;
    default: break;
    }
}
#endif // BGKP_METRIC_LOG_ENABLED
...
#if BGKP_METRIC_LOG_ENABLED
/*
    uint32_t tx_time = (clock_time()*1000)/CLOCK_SECOND;
    bgkp_fields_t f;
    const uint8_t *pl;
    uint16_t pl_len;
...
*/
    ...
    metrics_tx_from_bgkp_frame(ctx->buf, ctx->len);
...
/* ------------------------------------------------------------------ */
/* Emergency (harsh braking) parameters                                */
/* ------------------------------------------------------------------ */
/* 3.0 m/s^2 = 30 dm/s^2 (since 1 m = 10 dm) */
#ifndef HB_THR_DMPS2
#define HB_THR_DMPS2 40
#endif

/* Receiver-side and self-side emergency lifetime (TTL) */
#ifndef EF_TTL_MS
#define EF_TTL_MS 2000u
#endif

/* Bound emergency updates to avoid excessive overhead (matches 1 Hz LOC) */
#ifndef EF_TX_MIN_MS
#define EF_TX_MIN_MS 1000u
#endif

/* Force the red LED on for a visible window upon reception (independent of TTL) */
#ifndef EF_LED_FORCE_MS
#define EF_LED_FORCE_MS 5000u
#endif

static uint16_t v_prev_dmps = 0;
static uint32_t ef_self_expire_ms = 0;
static uint32_t ef_self_last_tx_ms = 0;
static uint8_t  ef_led_force = 0;
//static struct ctimer t_ef_led_force;
...
static struct ctimer t_ack_led;  /* ACK LED timer */
...
/*  lorn_mode:     indicates that there are no peers in the peer list */
/*  lonelink_mode: indicates that there's only one reliable friend    */
/* ------------------------------------------------------------------ */
static uint32_t friend_id[2]   = { BGKP_FRIEND_EMPTY, BGKP_FRIEND_EMPTY };
static uint8_t  friend_miss[2] = { 0, 0 };


static uint8_t lorn_mode = 0;       /* 0 friends */
static uint8_t lonelink_mode = 0;   /* exactly 1 stable friend */
...
typedef struct {
    ...
    uint8_t  friend_slots_empty_at_creation; /* Friend list state flag */
} data_retry_ctx_t;
...
static void
ack_led_off_cb(void *ptr)
{
    (void)ptr;
    bgkp_port_led_green_set(0);
}
...
/* Keep LED on if any emergency is active OR if force-window is active. */
static void
ef_led_refresh(void)
{
    ef_led_set((ef_led_force || ef_count_active()) ? 1 : 0);
}

/*
static void
ef_led_force_off_cb(void *ptr)
{
    (void)ptr;
    ef_led_force = 0;
    ef_led_refresh();
...
static void
ef_prune_expired(uint32_t now_ms)
{
    uint8_t changed = 0;
    for(uint8_t i = 0; i < EF_TAB_MAX; ++i) {
        if(ef_tab[i].in_use && now_ms >= ef_tab[i].expire_ms) {
            ef_tab[i].in_use = 0;
            changed = 1;
        }
    }
    if(changed) ef_led_refresh();
}
...
static void
ef_add_or_update(uint32_t origin, uint8_t code, uint32_t now_ms)
{
    if(code == BGKP_EF_NONE) return;
    int8_t idx = ef_find(origin);
    if(idx >= 0) {
        ef_tab[idx].code = code;
        ef_tab[idx].tick = ++ef_tick;
              ef_tab[idx].expire_ms = now_ms + EF_TTL_MS;
        return;
    }
    int8_t free_i  = -1;
    ...
    ef_tab[put].tick   = ++ef_tick;
       ef_tab[put].expire_ms = now_ms + EF_TTL_MS;
}
...
/* uart1_puts_ln                                                      */
/* What:                                                              */
/*     Compatibility wrapper for legacy call sites in mbl.c.          */
/*                                                                    */
/* Methods:                                                           */
/*     Route old UART prints through bgkp_port instead of uart1.      */
/*                                                                    */
/* Creates:                                                           */
...
static uint8_t is_my_friend(uint32_t id)
{
...
{
    /* Disabling Carry Slot Eviction Gossip */
    return;
    uint8_t buf[BGKP_MAX_FRAME];
    uint16_t out_len = 0;
    bgkp_fields_t f;
            ...
            uint8_t  ef_code)
{
    if(carry.count >= BGKP_CARRY_MAX_ITEMS) {
        ...
        if(old->in_use) {
            carry_build_and_gossip_slot(old); // Disabled Carry Eviction Gossip in the function
            carry_log_evict(old);
            carry_evicted++;
#if BGKP_METRIC_LOG_ENABLED
            metrics_agg_event(METR_EV_BUF_EVICT, 0, 0, 0, (uint16_t)carry.count);
#endif // BGKP_METRIC_LOG_ENABLED
        }
    }
...
#if BGKP_METRIC_LOG_ENABLED
            metrics_agg_event(METR_EV_BUF_PUT, 0, 0, 0, (uint16_t)carry.count);
#endif // BGKP_METRIC_LOG_ENABLED
...
*/
        ...
        if(s->msg_type == BGKP_MSG_EMERGENCY)
...
/* -------------------------------------------------------------------- */
/* Updates "friend modes"                                               */
/* -------------------------------------------------------------------- */
static void update_friend_modes(void)
{
    uint8_t count =
        (friend_id[0] != BGKP_FRIEND_EMPTY) +
        (friend_id[1] != BGKP_FRIEND_EMPTY);

    lorn_mode = (count == 0);

    lonelink_mode =
        (count == 1); // &&
//        ((friend_id[0] != BGKP_FRIEND_EMPTY && friend_miss[0] == 0) ||
//         (friend_id[1] != BGKP_FRIEND_EMPTY && friend_miss[1] == 0));
}
...
data_ack_window_cb(void *ptr)
{
    ...
    /* Step 1: promote candidates only is slots were emptyat DATA creation. */
    if(dctx.friend_slots_empty_at_creation){
        if(!friend_list_full()) {
            for(int i = 0; i < 2 && i < (int)cand_seen; ++i) {
                for(int c = 0; c < cand_seen; c++) {
                    // skip duplicates
                    int already = 0;
                    for(int f = 0; f < 2; f++) {
                        if(friend_id[f] == cand_id[c]) {
                            already = 1;
                            break;
                        }
                    }
                    ...
                    // find empty slot
                    for(int f = 0; f < 2; f++) {
                        if(friend_id[f] == BGKP_FRIEND_EMPTY) {
                            friend_id[f] = cand_id[c];
                            break;
                        }
                    }
                }
...
*/
    ...
    update_friend_modes(); // Update the state of the friend list to switch into appropriate mode if needed.
...
#if BGKP_METRIC_LOG_ENABLED
    metrics_agg_event(METR_EV_TX_ACK, 0, 0, 0, 0);
#endif // BGKP_METRIC_LOG_ENABLED
...
/*
 * send_emergency()
 * What: Builds and broadcasts an EMERGENCY frame and returns msg_id.
 * Why: Needed to log METR_EV_EMG_CREATE with the real message ID.
 * Creates: local payload buf and packed frame buf.
 */
static uint32_t
send_emergency(uint8_t ef_code)
{
    ...
    uint8_t buf[BGKP_MAX_FRAME];
    uint16_t out_len = 0;
    if(!bgkp_pack(&f, buf, sizeof(buf), &out_len)) return(0);
...
parse_loc_line(const char *line)
{
    ...
    /* -------------------------------------------------------------- */
    /* New emergency logic: harsh braking => send/update + TTL reset  */
    /* -------------------------------------------------------------- */
    /* Acceleration in dm/s^2: a = (dv * 1000) / dt_ms */
    int32_t dv_dmps = (int32_t)v_dmps - (int32_t)v_prev_dmps;
    int32_t a_dmps2 = (dv_dmps * 1000) / (int32_t)dt_ms;

    /* Prune any expired received emergencies using current timestamp. */
    ef_prune_expired(ts);

    if(a_dmps2 <= -(int32_t)HB_THR_DMPS2) {
        /* Extend local emergency TTL even if we don't transmit due to rate-limit. */
        ef_self_expire_ms = ts + EF_TTL_MS;

        /* Transmit at most once per EF_TX_MIN_MS, but always allow the first send. */

        if(!ef_active || (ts - ef_self_last_tx_ms) >= EF_TX_MIN_MS) {
//            uint8_t was_active = ef_active;
#if BGKP_METRIC_LOG_ENABLED
            uint32_t emg_msg_id = send_emergency(1); /* Emergency Braking code */
#else  // NOT BGKP_METRIC_LOG_ENABLED
            send_emergency(1);
#endif  // BGKP_METRIC_LOG_ENABLED
            ef_self_last_tx_ms = ts;

            if(!ef_active) {
                ef_active = 1;
                ef_add_or_update(origin_id, 1, ts);
                ef_led_refresh();
                ...
                /* Refresh our own entry TTL. */
                ef_add_or_update(origin_id, 1, ts);
            }
        }
    }

    /* Local emergency expires after EF_TTL_MS without a new Emergency event. */
    if(ef_active && ts >= ef_self_expire_ms) {
        ef_active = 0;
        ef_remove(origin_id);
        ef_led_refresh();
    }
...
/* mobile_uart_line_cb                                                */
/* What:                                                              */
/*     Receive complete UART lines from bgkp_port.                    */
/*                                                                    */
/* Methods:                                                           */
/*     Forward line input directly to the existing BGKP location      */
/*                                                            parser. */
/*                                                                    */
/* Creates:                                                           */
/*     No persistent local variables.                                 */
/* ------------------------------------------------------------------ */
static void
mobile_uart_line_cb(const char *line)
{
    if(line == NULL) {
        return;
    }

    (void)parse_loc_line(line);
}
...
start_gossip_data(void)
{
    ...
    dctx.friend_slots_empty_at_creation =
        (friend_id[0] == BGKP_FRIEND_EMPTY ||
         friend_id[1] == BGKP_FRIEND_EMPTY);
...
*/
    ...
    /* Metrics: per-message CREATE (batched) */
    metrics_agg_event(METR_EV_MBL_CREATE, origin_id, dctx.msg_id,
                      metrics_agg_now_ms(), 0);
          ...
          const uint8_t *data, uint16_t datalen)
{
    ...
    if(h.marker[0]=='R' && h.marker[1]=='S' && h.marker[2]=='U') {
        ...
        if(pl && pl_len >= 14) {
            ...
            uint32_t now_ms = (uint32_t)((clock_time() * 1000u) / CLOCK_SECOND);
            if(ef_origin != 0) {
                if(ef_code == BGKP_EF_FINISH)      ef_remove(ef_origin);
                else if(ef_code != BGKP_EF_NONE)   ef_add_or_update(ef_origin, ef_code, now_ms);
                ef_led_refresh();
            }
        }
...
#if BGKP_METRIC_LOG_ENABLED
            metrics_agg_event(METR_EV_BUF_RM, 0, 0, 0, (uint16_t)carry.count);
#endif // BGKP_METRIC_LOG_ENABLED
        }
        ...
        /* GREEN LED on ACK */
        bgkp_port_led_green_set(1);
        ctimer_set(&t_ack_led, ms_to_ticks_ceil(150),
                   ack_led_off_cb, NULL);
            ...
            if(pl && pl_len >= 9) {
                ...
                if(acked_type   == BGKP_MSG_DATA &&
                   ...
                   dctx.active) {
                    ...
                    /* ----------------------------------------------------------
                     * ACK quorum decision (Full / Lonelink modes)
                     * ---------------------------------------------------------- */
                    if(
                        /* Full mode: need ACKs from both friends */
                        (!lonelink_mode && dctx.ack_mask == 0x03u) ||

                        /* Lonelink mode: one stable friend is enough */
                        ( lonelink_mode && dctx.ack_mask != 0)
                    )
                    {
                        /* Delivery confirmed */
                        dctx.active = 0;
                
                        /* Remove DATA from carry store */
                        bgkp_carry_ack(&carry, origin_id, dctx.msg_id);
    ...
    if(h.msg_type == BGKP_MSG_QUERY) {
        ...
        if(pl && pl_len >= 5) {
            ...
            else                          ef_add_or_update(ef_orig, ef_flag, now_ms);
            ef_led_refresh();
#if BGKP_DIAG
    ...
    if(h.msg_type == BGKP_MSG_DATA) {
        ...
        /* Ignore if sender is our friend to diversify the friend list. */
...
*/
    ...
    if(h.msg_type == BGKP_MSG_EMERGENCY) {
        ...
        /* Local reception time (must be visible for RX + FWD). */
        uint32_t now_ms =
            (uint32_t)((clock_time() * 1000u) / CLOCK_SECOND);

        /* EF code must be visible for FWD log as well. */
        uint8_t efc = 0;
        uint8_t valid_emg = 0;
        ...
        if(pl && pl_len >= 5) {
            ...
            /* Treat any received emergency as start/update and reset TTL. */
            ef_prune_expired(h.ts_ms);
            ef_add_or_update(h.origin_id, efc, h.ts_ms);

#if BGKP_METRIC_LOG_ENABLED
            /* Per-message: Emergency reception for MET reports. */
            metrics_agg_event(METR_EV_EMG_RX,
                              h.origin_id,
                              h.msg_id,
                              now_ms,
                              origin_id);   // receiver node
#endif

            /* Force LED on for a visible window. */
            /* Commenting out for emergency LED usage
            ef_led_force = 1;
            ef_led_refresh();
            ctimer_set(&t_ef_led_force,
                       ms_to_ticks_ceil(EF_LED_FORCE_MS),
                       ef_led_force_off_cb, NULL);
             */
        }
        ...
        /* Forward only one EF per origin within TTL window */
        uint8_t do_forward = 1;
        int8_t idx = ef_find(h.origin_id);

        if(idx >= 0) {
            if(now_ms < ef_tab[idx].last_forward_ms + EF_TTL_MS) {
                do_forward = 0;
           } else {
                ef_tab[idx].last_forward_ms = now_ms;
            }
        }

        if(do_forward && valid_emg) {
#if BGKP_METRIC_LOG_ENABLED
...
PROCESS_THREAD(mobile_process, ev, data)
{
    ...
    bgkp_port_init(&mobile_process, mobile_uart_line_cb);

    /*
     * For script-driven LOC lines it is usually better to disable
     * local echo to avoid cluttering the UART output.
     */
    bgkp_port_local_echo_enable(0);
    button_hal_init();
    ...
    while(1) {
#if BGKP_METRIC_LOG_ENABLED
        metrics_agg_poll(metrics_agg_now_ms(), (uint16_t)origin_id, 0);
#endif
        PROCESS_YIELD();

        if(bgkp_port_handle_event(ev, data)) {
            continue;
        }

        uint32_t now_ms_main = (uint32_t)((clock_time() * 1000u) / CLOCK_SECOND);
        /* ---- Button events ---- */
        ...
        if(ev == button_hal_release_event) {
            btn_pressed = 0;
            if(ef_active) {
                ef_self_expire_ms = now_ms_main + EF_BUTTON_TIMEOUT_MS;
            }
        }

        /* ---- Button handling ---- */
        ef_prune_expired(now_ms_main);

        if(btn_pressed &&
           now_ms_main - ef_self_last_tx_ms >= EF_BUTTON_TX_PERIOD_MS) {
            send_emergency(BGKP_EF_BUTTON);
            ef_self_last_tx_ms = now_ms_main;
        }

        /* Expire emergency only after button release AND timeout */
        if(ef_active &&
           !btn_pressed &&
           ef_self_expire_ms != 0 &&
           now_ms_main > ef_self_expire_ms) {
            ef_active = 0;
            ef_self_expire_ms = 0;
        ...
        /* RED LED tied to EF */
        bgkp_port_led_red_set(ef_active);

        if(etimer_expired(&t_loc)) {
            uart1_puts_ln("REQ_LOC");
            bgkp_port_heartbeat_enable(0);
            //bgkp_port_led_green_toggle();
