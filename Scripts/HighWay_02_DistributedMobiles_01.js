...
/* -------------------------- Parameters ------------------------------------ */
var STATIONARY_SPACING = 1000;
var MAX_X = 5000; /* absolute bound along X axis */
...
var DELTA_SPEED = 2; /* small jitter per tick */
...
/* Random uniform placement along highway */
for (var k = 0; k < mobileMotes.length; k++) {
    var mm = mobileMotes[k];
    var mid = mm.getID();
    var pos = mm.getInterfaces().getPosition();

    /* Uniform distribution across [0, MAX_X] */
    var startX = Math.random() * MAX_X;

    pos.setCoordinates(startX, 0, 0);

    /* Direction: keep your parity logic */
    var sign = ((mid % 2) === 1) ? +1 : -1;
...
function scheduleNextMove() {
  GENERATE_MSG(SIMULATION_TICK_DELAY, "move_next"); // Move motes again for \delta V
}
scheduleNextMove();
...
while (true) {
  ...
  /* ================== Check that the test timer is not timed out =========== */
  if (time >= END_TIME_MS) {
  ...
  if (typeof msg === "string" && msg === "move_next") {
    for (var i = 0; i < mobileMotes.length; i++) {
      ...
      var dt_s = SIMULATION_TICK_DELAY / 1000; // 1 second delay
      var dx = vabs * dt_s * moteSpeedSign[i];
      x += dx;
